...
These domain values may be change to a no element tensor .
...
If no_elements(dom_der[j]), then dom_der[j] represents a zero tensor
...
If no_elements(rng_der[i]), then rng_der[i] represents a zero tensor
