...
// ----------------------------------------------------------------------------
/*
{xrst_begin atom_ad_reverse_der usr}
{xrst_spell
    nowrap
    lll
}
...
Recording Reverse Derivatives That Use Atomic Functions
#######################################################
...
TEST(examples_atom, ad_reverse_der)  {
    ...
    // fp = g(v) = f'(v) = (3 * v * v)
    vector<adten_t> adr;
    adr.push_back( adten_t( torch::tensor( 1.0 ) ) );
    vector<adten_t> afp = f.reverse_der(adr, av_all);
    ...
    //
    // equal
    bool equal = fp[0].equal( 3.0 * v[0] * v[0] );
    EXPECT_TRUE(equal);
    //
    // dv, gp
    vector<Tensor> dv;
