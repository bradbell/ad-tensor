...
adfn_name.con
=============
The binary constant output file name begins with the name of this adfn;
and the ends with ``.con`` .
It contains the constant at::Tensors used to evaluate the
source code function defined below.

Source Code Function
********************
The C++ source defines the following function:
range = adfn(dom_var, dom_par) where:
{xrst_code cpp}
...
R"|(// {}_plugin
// This source code was generated by ad_tensor::adfn::src_gen.
#include <thread>
...
{{   //
    ...
    using ad_tensor::has_elements;
        ...
        src += std::format(fmt, adfn_name, adfn_name);
    }
    //
    // ifndef NDEBUG
    src += indent + "//\n";
...
R"|(    //
    ...
    //
    // file_cpp: con_vec
    {   string file_name = fs::canonical(file_con_path).string();
        constexpr const char* fmt =
R"|(    //
    // con_vec
    thread_local vector<Tensor> con_vec;
    if( con_vec.size() == 0 ) {{
        {{   string file_name = "{}";
...
//
BOOST_DLL_ALIAS({}_plugin, {})
)|";
        file_cpp << std::format(fmt, adfn_name, adfn_name, adfn_name);
    }
    file_cpp.close();
}
