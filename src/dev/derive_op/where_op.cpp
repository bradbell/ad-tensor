...
# endif
    ...
    if( true_type  == adtype_t::variable ) {
        ...
        // dim
        broadcast(
            var_all[op_index].sizes(), var_all[true_index].sizes(), array
        );
    ...
    if( false_type  == adtype_t::variable ) {
        ...
        // dim
        broadcast(
            var_all[op_index].sizes(), var_all[false_index].sizes(), array
        );
        c10::IntArrayRef dim(array);
        //
...
// src_gen
template <class TensorType>
std::string where_op_t<TensorType>::src_gen(
    size_t                                                op_index        ,
    const agraph_t&                                       agraph          ,
    bool                                                  variable_agraph ,
    const std::function< std::string(size_t, adtype_t) >& tensor_src
) const {
    user_assert(false, "src_gen not yet implemented for where operator" );
    return "";
}
template std::string where_op_t<adten_t>::src_gen(
    size_t                                                op_index        ,
    const agraph_t&                                       agraph          ,
    bool                                                  variable_agraph ,
    const std::function< std::string(size_t, adtype_t) >& tensor_src
) const;
template std::string where_op_t<at::Tensor>::src_gen(
    size_t                                                op_index        ,
    const agraph_t&                                       agraph          ,
    bool                                                  variable_agraph ,
    const std::function< std::string(size_t, adtype_t) >& tensor_src
