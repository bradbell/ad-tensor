...
# endif
    ...
    if( lhs_zero_der ) {
        ...
        // for_der
        for_der[op_index] =
            var_all[lhs_index] * for_der[rhs_index] +
            for_der[lhs_index] * var_all[rhs_index];
    };
}
...
// ---------------------------------------------------------------------------
// src_gen
template <> std::string mul_op_t<at::Tensor>::src_gen(
    size_t                                                op_index        ,
    const agraph_t&                                       agraph          ,
...
template <> std::string mul_op_t<adten_t>::src_gen(
    ...
    assert(false && "adten_t version of src_gen called for mul operator");
    return "";
}
} } // End ad_tensor::dev
